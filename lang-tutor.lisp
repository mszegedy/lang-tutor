...
;;;; - Adheres to Google's Common Lisp Style Guide (mostly, even that rule about line wrapping)
...
;;; Constants ;;;
;;;;;;;;;;;;;;;;;
(defconstant +non-printing-words+ '(<blank> <close-quote> <combiner> <combining-apostrophe> <comma>
                                    <exclamation> <open-quote> <period> <question> <space>)
  "A list of symbols that aren't meant to be printed. This includes all meta-symbols.")
...
(defun process-file (filename)
  "Turns a file into a LESSON object."
  filename)
...
argument SIDE controls which side that is: 'LEFT will make it the left side, 'RIGHT will make it
the right side, and anything else, such as 'NEITHER, will do neither side. (Please don't use this
to deepcopy.)"))
...
(defgeneric get-link (environment
                      &key
                        ...
                        right-index)
  (:documentation
   "Gets the link with the associated information, if any. All of the keyword arguments must be
provided!"))
...
(defgeneric match-answer (environment answer sequence
                          &key
                            ...
                            sloppyp)
  (:documentation
   "Checks whether an answer is able to be generated by a particular sequence. The keyword
arguments are all optional. ANSWER-TYPE should be set to STRING when the answer is a string, or
SEQUENCE when the answer is a sequence. The rest of the arguments determine how strict the matching
is."))
(defgeneric expand-rule-sequence (environment &key left-sequence right-sequence link)
  (:documentation
   "Randomly expands a sequence, LEFT-SEQUENCE, into a list of symbols. A RIGHT-SEQUENCE and a LINK
may optionally be provided so that the RIGHT-SEQUENCE is expanded as well along with the
LEFT-SEQUENCE, to its most general possible form that may or may not still include RULE-POINTERs,
and returned as the second return value."))
;; First appear in LESSON
(defgeneric play (lesson)
  (:documentation
   "Activates an interactive prompt where the user learns the rules taught by
the lesson."))
...
(defclass rule ()
  ((name
   ...
   (sequences
    ...
    :initform nil
    :documentation
    "The list of possible sequences the rule can expand to. A list of lists of symbols and
  RULE-POINTERs."))
  (:documentation
   "A rule for making parts of sentences with. Expands to a sequence of rules and sentences."))
...
(defclass link ()
  ((left-name
   ...
   (left-index
    ...
    :initarg :left-index
    :documentation
    "The index of the sequence for the rule on the left. If it's a word, the index is 0.")
   (right-index
...
(defmethod copy-with-index-reset ((link link) &key side)
  (make-instance
   ...
   :right-index (if (eql side 'right)
                    0
                    (right-index link))))
(defmethod downgrade-mystery ((link link) &key (multiplier 0.95))
...
(defclass environment ()
  ...
  respectively, its LEFT-NAME, RIGHT-NAME, LEFT-INDEX, and RIGHT-INDEX."))
  (:documentation
...
(defmethod sequence-expansions ((environment environment) sequence)
  (let ((first-rule-pointer-index
          ;; If the loop finishes without finding a RULE-POINTER, it returns nil
          (loop
            for item in sequence
            for index from 0
...
(defmethod match-answer ((environment environment) answer sequence
  ...
  (let* ((expansions (sequence-expansions environment sequence))
         (expansion-strings (mapcar (lambda (expansion)
    ...
    (flet ((modify (string)
                 ...
                 (let* ((max-distance 2)
                   ...
                   (flet ((distance (x y)
                            (cond
                              ...
                              (t
                               (setf (aref rec x y)
                                     (+ (if (char= (char string-1 (- length-1 x))
                                                   (char string-2 (- length-2 y)))
                                            0
                                            1)
                                        (min (distance (1- x) y)
                                             (distance x (1- y))
      ...
      (member (modify answer)
              ...
              :test #'test))))
(defmethod expand-rule-sequence ((environment environment) &key left-sequence right-sequence link)
                ...
                (let* ((left-rule (get-rule environment (name left-item)))
                  ...
                  (cond
                    (best-link
                     ...
                     ;; The link tells the function what pair of sequences to use in the
                     ;; expansion. The one on the left is made part of the returned list, while the
                     ...
                     (multiple-value-bind (left-expansion right-expansion)
                          ...
                          :right-sequence (if (eql (right-name best-link) (right-name link))
                                              (list right-rule)
                                              (elt right-sequences (right-index best-link)))
                          :link (copy-with-index-reset
                                 best-link
                                 :side (cond
                                         ...
                                         (t
                                          'neither))))
                       (setf right-result (append (subseq right-result 0 right-index)
                                                  right-expansion
                    ...
                    (left-sequences
                     (expand-rule-sequence
                      environment
                      :left-sequence (elt left-sequences (random (length left-sequences)))))
                    (t
                     (debug-out
...
(defclass lesson ()
  ((environment
    ...
    :initarg :environment
    :documentation
    "The rules and links for this lesson.")
   (introduction
    :accessor introduction
    :initarg :introduction
    :initform nil
    :documentation
    "The text displayed at the beginning of the lesson.")
   (left-root-rule
    :accessor left-root-rule
    :initarg :left-root-rule
    :documentation
    "The rule on the left to start with when making sentences.")
   (right-root-rule
    :accessor right-root-rule
    :initarg :right-root-rule
    :documentation
    "The rule on the right to start with when making sentences.")
   (rules-to-learn
    :accessor rules-to-learn
    :initarg :rules-to-learn
    :initform nil
    :documentation
    "A list of the names of various rules to learn. This is a subset of all of the rules in the
    environment."))
  (:documentation
   "Stores a single language lesson to be learned."))
(defmethod play ((lesson lesson)))
